...
```{r npv_comparative_distributions, warning=FALSE}
library(bayesplot)

outcomes_df <- data.frame(
  `Decision NPV`       = simulation_result$y$decision_value,
  `Net Health Benefit` = simulation_result$y$net_health_benefit,
  `Education Benefit`  = simulation_result$y$education_benefit,
  check.names = FALSE
)

color_scheme_set("mix-blue-red")
mcmc_areas(outcomes_df, prob = 0.9, point_est = "median") +
  ggplot2::labs(x = "Million VND",
                title = "Distribution of discounted policy outcomes (90% interval)") +
  ggplot2::theme_minimal()
```

## Annual NPV components across the 5-year horizon

The following figure decomposes the NPV into its three annual components — health savings, education benefit, and implementation costs — discounted to present value for each year. Each boxplot summarises the distribution across all 10,000 simulations, showing how uncertainty in the inputs propagates through time. Costs are shown as negative values; health and education benefits as positive.

```{r npv_cashflow_timeline, warning=FALSE}
library(dplyr)
library(tidyr)
...
health_benefit   <- simulation_result$y$net_health_benefit
edu_benefit      <- simulation_result$y$education_benefit

recurring_costs  <- x$training_costs_foodsafety_annual +
                    x$training_costs_nutrition_annual +
                    x$training_costs_physical_activity_annual +
                    x$change_menu_costs_annual +
                    x$monitoring_canteen_cost

first_year_extra <- x$training_costs_foodsafety_1st_year +
                    x$training_costs_nutrition_1st_year +
                    x$training_costs_physical_activity_1st_year

cashflow_long <- lapply(1:n_years, function(t) {
  disc    <- 1 / (1 + dr)^t
  costs_t <- if (t == 1) recurring_costs + first_year_extra else recurring_costs
  data.frame(
    Year      = paste0("Year ", t),
    Health    =  health_benefit * disc,
    Education =  edu_benefit    * disc,
    Costs     = -costs_t        * disc
  )
}) %>%
  bind_rows() %>%
  pivot_longer(c(Health, Education, Costs),
               names_to  = "Component",
               values_to = "Value") %>%
  mutate(
    Year      = factor(Year, levels = paste0("Year ", 1:n_years)),
    Component = factor(Component, levels = c("Health", "Education", "Costs"))
  )

ggplot2::ggplot(cashflow_long,
                ggplot2::aes(x = Year, y = Value, fill = Component)) +
  ggplot2::geom_boxplot(outlier.shape = NA, alpha = 0.7, width = 0.6) +
  ggplot2::facet_wrap(~Component, scales = "free_y", ncol = 1) +
  ggplot2::scale_fill_manual(
    values = c(Health = "#2196F3", Education = "#4CAF50", Costs = "#F44336")
  ) +
  ggplot2::geom_hline(yintercept = 0, linetype = "dashed", colour = "grey40") +
  ggplot2::labs(x = NULL, y = "Million VND (discounted)",
                title = "Discounted annual NPV components across 5-year horizon") +
