...
All model inputs were modeled probabilistically based on expert judgment and literature estimates. See the input file `input_school_policy_post.csv` for full details.
...
The three outcomes tracked were, NPV (Net Present Value of policy vs. no policy), Health Cost Savings (avoided treatment and diagnosis costs), Cost per Student (total implementation cost per student). 
...
Before examining the Pareto front, we compare the distributions of the three discounted outcome variables directly. The Decision NPV integrates both health savings and education gains net of costs, while Net Health Benefit and Education Benefit represent the component contributions before cost deduction.

```{r npv_comparative_distributions, warning=FALSE, echo=FALSE}


plot_distributions(mcSimulation_object = simulation_result, 
...
Each row is one intervention (active runs only). The three columns show the distribution of the **overall net** NPV, the **health** component, and the **education** component across all Monte Carlo runs. Box spans the interquartile range; whiskers extend to 5th and 95th percentiles.
...
Overall our simulations show that implementation of the policy will have a positive effect on health and education. Which interpretations and practical interventions based on the policy will be most effective can be found in the simulations. Next we look for formulations of school meal interventions that show the greatest influence on overall, health and education outcomes. Our model includes toggle variables to simulate the presence or absence of specific interventions. We can identify which combinations of actions appear in the most efficient scenarios.

# Comparing five actions

## Mean decision value by action

As a first step, we compare the mean net present value (NPV) of the policy decision across simulations where each intervention was present (toggle = 1) versus absent (toggle = 0). This provides a direct, interpretable summary of the average outcome difference associated with each action in isolation.
...
The results reveal a clear hierarchy among the five actions. Limiting unhealthy canteen food shows the largest individual effect — simulations where this intervention was active produced a mean NPV approximately `r round((8.0 - 5.3) / 5.3 * 100)`% higher than those where it was absent (difference: `r round(7962 - 5313)` million VND). Physical activity and nutrition training show moderate positive differences of roughly 1,000–1,100 million VND each (~13–14%), while food safety training has a small marginal effect (~130 million VND, ~2%). Notably, menu change to meet RDA shows a **negative** mean difference (-393 million VND) — simulations where it was active produced slightly lower NPV on average, suggesting that in the current parameterisation the menu change costs are not fully offset by the health or education gains modelled for this intervention. This does not mean the intervention is harmful, but rather that its implementation costs may outweigh the modelled within-school benefits at this scale.

However, individual toggle comparisons cannot capture how actions interact when implemented together. In this model, benefits compound through intermediate variables (BMI, food scores, disease reduction), so the effective return of any action depends in part on which other actions are also active. A scenario with all five toggles on is not simply the sum of five individual differences. Identifying which *combinations* yield the most efficient outcomes — and where trade-offs arise between NPV, health, and education benefits — requires a multi-objective approach. The Pareto analysis below addresses this.
...
According to the EVPI there is value in gathering further information to better inform decision making.
...
before committing to a decision. For **net health benefits**, the EVPI is
`r round(sum(EVPI$net_health_benefit$EVPI_do, na.rm = TRUE), 3)` million VND, meaning
that uncertainty in the modelled inputs has no bearing on the health-related decision:
the intervention yields a positive health outcome regardless of which plausible input
values are drawn. For **education benefits**, the EVPI is
`r round(sum(EVPI$education_benefit$EVPI_do, na.rm = TRUE), 4)` million VND —
effectively zero — indicating the education benefit is similarly robust to parameter
uncertainty. For the **composite decision value** (NPV), the EVPI is
`r round(sum(EVPI$decision_value$EVPI_do, na.rm = TRUE), 2)` million VND, reflecting
a small residual uncertainty in the economic return: a minority of simulations produce
...
These simulations likely echo nutrition education or physical activity components that improve cognitive and academic outcomes, which strengthens the policy’s value. Better student performance aligns with optimal outcomes. Improvements in learning outcomes contribute to broader benefits, supporting higher returns in the model.
...
During expert elicitation, staff at each of the three study schools provided their own estimates of health-related parameters — specifically the baseline frequency of disease diagnosis and treatment visits, the likely reduction in those events under the policy, and the associated costs per event. These school-level estimates were not used in the main Monte Carlo simulation (which draws on the pooled `input_school_policy_post.csv`), but they offer a valuable independent perspective on the expected scale of health benefits.

Notably, the school experts generally gave substantially higher values for both baseline disease burden and cost per event than the conservative pooled estimates: baseline diagnosis rates of 1–4 visits per student per year (vs. 0.01–0.03 in the main model) and per-event costs of 0.2–5 million VND (vs. 0.005–0.05 million VND). If these school-level estimates are taken at face value, the expected health effect of the policy is considerably larger.

The analysis below samples from each school's distributions and computes:

- **Baseline annual health cost** — the expected cost burden without the policy (diagnoses + treatments × cost per event × number of students).
- **Avoided annual health cost** — the expected health saving attributable to the policy (reduced events × cost per event × number of students), equivalent to the `net_health_benefit` output in the main model.
- **Proportional reduction** — avoided cost as a percentage of baseline.

```{r school_health_expert, warning=FALSE, echo=FALSE, message=FALSE}

school_files <- c(
  "School 4-1"   = "data/three_schools_data/input_school_policy_4_1.csv",
...
n_expert_samples <- 5000

# Mini model function: only uses the health-related inputs from each school CSV.
# mcSimulation handles all distribution sampling, avoiding raw random() return-type issues.
health_metrics_function <- function() {
  diag_cost  <- max(0, disease_diagnosis_cost)
  treat_cost <- max(0, disease_treatment_cost)

  baseline_cost <- (baseline_disease_diagnosis * diag_cost +
                    baseline_disease_treatment  * treat_cost) * n_student

  avoided_cost  <- (n_reduce_disease_diagnosis * diag_cost +
                    n_reduce_disease_treatment  * treat_cost) * n_student

  pct_reduction <- ifelse(baseline_cost > 0, avoided_cost / baseline_cost * 100, NA_real_)
...
set.seed(42)
school_health_list <- lapply(names(school_files), function(school_name) {
  sim <- mcSimulation(
    estimate         = estimate_read_csv(school_files[[school_name]]),
    model_function   = health_metrics_function,
    numberOfModelRuns = n_expert_samples,
    functionSyntax   = "plainNames"
  )
  data.frame(
    school        = school_name,
    baseline_cost = sim$y$baseline_cost,
    avoided_cost  = sim$y$avoided_cost,
    pct_reduction = sim$y$pct_reduction
  )
})

school_health_df <- do.call(rbind, school_health_list)
school_health_df$school <- factor(school_health_df$school, levels = names(school_files))
```

## Baseline health burden and avoided costs by school

```{r school_health_plot, warning=FALSE, echo=FALSE, fig.height=6, fig.width=10}

long_df <- tidyr::pivot_longer(
  school_health_df,
  cols      = c(baseline_cost, avoided_cost),
  names_to  = "metric",
  values_to = "value_mVND"
)
long_df$metric <- factor(long_df$metric,
                         levels = c("baseline_cost", "avoided_cost"),
                         labels = c("Baseline health cost (no policy)",
                                    "Avoided health cost (policy benefit)"))

ggplot(long_df, aes(x = school, y = value_mVND, fill = metric)) +
  geom_boxplot(outlier.shape = NA, position = position_dodge(width = 0.75),
               alpha = 0.7, coef = 1.5) +
  scale_fill_manual(values = c("Baseline health cost (no policy)" = "salmon",
                                "Avoided health cost (policy benefit)" = "steelblue")) +
  labs(
    x    = NULL,
    y    = "Million VND per year",
    fill = NULL,
    title = "Expected annual health costs and policy savings by school\n(based on school experts' estimates)"
  ) +
  theme_minimal(base_size = 11) +
  theme(legend.position = "bottom")
```

## Proportional reduction in health costs

```{r school_health_pct_plot, warning=FALSE, echo=FALSE, fig.height=4, fig.width=8}

ggplot(school_health_df, aes(x = school, y = pct_reduction, fill = school)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.7, coef = 1.5, show.legend = FALSE) +
  geom_hline(yintercept = 50, linetype = "dashed", colour = "grey50") +
  scale_fill_brewer(palette = "Set2") +
  labs(
    x     = NULL,
    y     = "Avoided cost as % of baseline",
    title = "Proportional reduction in health costs under the policy\n(school experts' estimates)"
  ) +
  theme_minimal(base_size = 11)
```
...
```{r school_health_summary, warning=FALSE, echo=FALSE}

summary_health <- school_health_df %>%
  dplyr::group_by(school) %>%
  dplyr::summarise(
    Baseline_median_mVND  = round(median(baseline_cost,  na.rm = TRUE), 1),
    Avoided_median_mVND   = round(median(avoided_cost,   na.rm = TRUE), 1),
    Avoided_p10_mVND      = round(quantile(avoided_cost, 0.10, na.rm = TRUE), 1),
    Avoided_p90_mVND      = round(quantile(avoided_cost, 0.90, na.rm = TRUE), 1),
    Pct_reduction_median  = round(median(pct_reduction,  na.rm = TRUE), 1),
    .groups = "drop"
  )

knitr::kable(summary_health,
             col.names = c("School", "Baseline (median, M VND/yr)",
                           "Avoided cost (median)", "Avoided cost (P10)", "Avoided cost (P90)",
                           "% reduction (median)"),
             caption = "Annual health cost burden and expected policy savings by school, based on school experts' own estimates.")
```

These school-level estimates suggest that the health benefit of the policy could be considerably larger than reflected in the pooled model — the gap is driven primarily by higher baseline disease rates and higher per-event costs reported by school staff. The proportional reduction (policy-attributable avoided costs as a share of baseline) gives a consistent signal across schools regardless of scale differences. This supplementary assessment does not change the main model conclusions, but it indicates that the health dimension of the intervention may be meaningfully understated when using the conservative pooled estimates, and supports targeted further data collection at the school level.
