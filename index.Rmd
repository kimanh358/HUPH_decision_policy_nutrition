...
The results reveal a clear hierarchy among the five actions. Limiting unhealthy canteen food shows the largest individual effect — simulations where this intervention was active produced a mean NPV approximately 43% higher than those where it was absent. Menu change to meet RDA, food safety training, and physical activity show moderate positive differences (~8–9%), while nutrition training has a negligible individual effect (~1%). All five actions show a positive direction (mean NPV is higher when on than off), confirming that each intervention contributes on average, though to very different degrees.
...
According to the EVPI there is no value in gathering further information to better inform decision making. No expected losses in any of the simulations. 
...
before committing to a decision. For **education benefits**, the EVPI is
`r round(sum(EVPI$education_benefit$EVPI_do, na.rm = TRUE), 2)`, meaning that
uncertainty in the modelled inputs has no bearing on the education-related decision:
the intervention yields a positive education outcome regardless of which plausible
input values are drawn. For **net health benefits**, the EVPI is
`r round(sum(EVPI$net_health_benefit$EVPI_do, na.rm = TRUE), 2)`, indicating a small
but non-zero expected gain from obtaining perfect information — some parameter
combinations produce borderline health outcomes, but the overall magnitude is low
relative to the scale of benefits. For the **composite decision value** (NPV), the
EVPI is `r round(sum(EVPI$decision_value$EVPI_do, na.rm = TRUE), 2)` (million VND),
reflecting minor residual uncertainty in economic returns; the decision to implement
is robust but a modest portion of simulations sits near break-even. Together, the low
EVPI values across all three outcomes suggest that further data collection is unlikely
to change the policy recommendation, and the current evidence base is sufficient to
