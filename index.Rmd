...
# Point-biserial correlation of five actions

Since `chance_event()`returns 0/1, `cor` of `results` gives us a point-biserial correlation of each policy toggle with the decision outcome across all runs. For each policy toggle, point-biserial correlation compares the distribution of decision_value in runs where that intervention fired (1) vs. didn't fire (0). A value of +1 would mean the intervention always produced higher decision values; -1 always lower.

```{r cor_results}
cor(simulation_result$x[, c("use_staff_training_foodsafety",
                            "use_staff_training_nutrition",
                   "use_physical_activity",
                   "use_menu_change_rda",
                   "use_limit_unhealthy_canteen_food")],
    simulation_result$y$decision_value)
```
The correlations of the five actions are all essentially zero (range: -0.016 to +0.016). No single intervention strongly drives the decision outcome on its own — the interventions are not individually decisive. The negative values for `use_staff_training_foodsafety` and `use_physical_activity` are counterintuitive but not significant — they're noise at this magnitude. This is consistent with our model's logic: benefits compound through intermediate variables (BMI, food scores, disease reduction), and the `use_`* flags are just one layer of a multi-step causal chain. Why this happens mechanically: Each `use_`* variable is sampled ~uniformly from 0.01–0.99, so `chance_event` fires roughly 50% of the time for all of them. With all toggles varying simultaneously and independently, the variance in decision_value is dominated by the effect-size parameters (`physical_activity_effect`, `menu_change_rda_nutrition_effect`, etc.) and structural parameters (`n_student`, `disease_treatment_cost`, etc.) — not by which toggle fired.
...
```{r test_evpi_if_should_plot}
# "decision_value", "net_health_benefit", "education_benefit"
sum(EVPI$decision_value$expected_gain, na.rm = T) <0 # False do not plot
sum(EVPI$net_health_benefit$expected_gain, na.rm = T) <0 # False do not plot
sum(EVPI$education_benefit$expected_gain, na.rm = T) <0 # False do not plot
```
